...
#### Activity (пошаговая логика)
 ...
 - if / else — проверка условия, разделение потоков.

 - repeat — повторяющиеся действия (например, проверка списка броней).
...
### В каком порядке изучать
 ...
 1. Начать с пользовательских историй (User Stories)

 2. Сначала CLI-примеры, чтобы понять, как система выглядит пользователю.
